...
func (r *repository) AddTicket(ticket models.Ticket) (models.Ticket, error) {
	err := r.db.Preload("Station").Save(&ticket).Error
...
func (r *repository) FilterStation(statiun int) (models.Station, error) {
	...
	err := r.db.Where("station = ?", station).First(&station).Error
...
func (r *repository) FilterTicket(start int, destination int, date string) ([]models.Ticket, error) {
	var ticket []models.Ticket
	err := r.db.Debug().Preload("StartStation").Preload("DestinationStation").Where("start_destination_id = ? AND destination_station_id = ? AND start_date = ?", start, destination, date).Find(&ticket).Error
